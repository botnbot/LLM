...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if created_user:
            ...
            self.stdout.write(
                self.style.WARNING(f"Пользователь {user.email} уже существует")
            )
        ...
        if created_course:
            ...
            self.stdout.write(
                self.style.WARNING(f"Курс '{course.name}' уже существует")
            )
        ...
        if created_lesson:
            ...
            self.stdout.write(
                self.style.WARNING(f"Урок '{lesson.name}' уже существует")
            )
